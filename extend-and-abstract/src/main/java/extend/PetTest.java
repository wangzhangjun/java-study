 ...
 *               其他构造方法，最好将无参构造方法写上
 *       7、如果构造方法中显式的指定了super的构造方法，那么父类的无参构造方法就不会被调用
 *
 *   总结：
 ...
 *   构造方法不会被重写，因为构造方法不能被继承
 *   父类的静态方法子类可以进行调用（可以调用），但是子类不可以重写
 * */

public class PetTest {
    public static void main(String[] args) {
        Dog dog = new Dog("xiao", 12, "nan", "wangwang");
        dog.play();

        Dog dog1 = new Dog();  //会默认调用Dog和Pet无参构造方法,对应上面的第6点
